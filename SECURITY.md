...
When using `@beefree.io/vue-email-builder`:
...
In Vue, use a composable or `onMounted` to fetch the token from your API; never store credentials in `.env` files that are bundled into the client.
...
Regularly update `@beefree.io/vue-email-builder` and its dependencies:

```bash
yarn upgrade @beefree.io/vue-email-builder
```
